...
sim_iter <- function(d_, y, x, outlier) {
  ...
  i <- sample(avail_indices, replace = FALSE, size = 116)
...
sort(y)[(length(y)-1):length(y)]
which(y > 18.40109) # 256 3686
y[c(256,3686)]
x[c(256,3686)]

sort(y)[1:2]
which(y < 1.386295) # 2969  4609
x[c(4609,2969)]
y[c(4609,2969)]
...
# example of a sample
outlier <- c(256,3686,4609,2969)
y_without_outlier <- y[-outlier]
x_without_outlier <- x[-outlier]
indices <- 1:N
avail_indices <- indices[-outlier]
i <- sample(avail_indices, replace = FALSE, size = sample_size[1]-4)
y_sample <- c(y_without_outlier[i], y[outlier])
x_sample <- c(x_without_outlier[i], x[outlier])
